...
<script>
import { getRoleListAPI, getCompanyInfoAPI, addRoleAPI, getRoleDetailAPI, getProfileAPI, updateRoleAPI, deleteRoleAPI, getPermissionListAPI, assignPermAPI } from '@/api'
...
export default {
  ...
  data() {
    return {
      ...
      isEdit: false, // 新增/编辑-角色状态(false新增，true编辑)
      dialogVisible: false, // 显示/隐藏->分配权限点的弹框
      permissionList: [], // 所有权限点数据(树形结构)
      permIds: [], // 此角色现有的权限点数据(权限点id值字符串时)
      clickRoleId: '' // 分配权限时，点击的那行角色id
    }
  },
  created() {
    // 获取所有角色列表
  ...
  methods: {
    ...
    async setRoles(roleObj) {
      ...
      this.permIds = res.data.permIds // 字段值保存之前分配权限点的id值
    },
